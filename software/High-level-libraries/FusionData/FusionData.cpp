...
void ComputePositionWithDR(PositionData &oldData, PositionData &newData)
{
	...
	if ( newData.time_ms > oldData.time_ms) {
        PositionData temp;
        CopyData(temp, newData);
        
        // Calculate distance
		newData.distance_mm = (newData.time_ms - oldData.time_ms) * ((newData.speed_mmPs) / 1000.0);
		...
		newData.x_Pos_mm = oldData.x_Pos_mm + cos((newData.bearing_deg / HEADING_PRECISION) * TO_RADIANS) * newData.distance_mm;
		newData.y_Pos_mm = oldData.y_Pos_mm + sin((newData.bearing_deg / HEADING_PRECISION) * TO_RADIANS) * newData.distance_mm;
        
        CopyData(oldData, temp);
